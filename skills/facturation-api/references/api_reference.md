...
|:-------|:--------|:-----|:------|
| id | Identifiant| int | lecture |
| supplier_id | Fournisseur| int | modifiable |
...
| payment_mode | Mode de règlement| int | modifiable |
...
| serial_number | Numéro de pièce| int | lecture |
| category_id | Catégorie| int | modifiable |
...
| nature | Nature de l'achat| int | modifiable |
...
| api_id | API - Référence numérique libre| bigint | modifiable |
...
| status | Affectation| tinyint | modifiable |
| created_at | Créé le| datetime(3) | lecture |
| updated_at | Modifié le| datetime(3) | lecture |
...
liste des 50 devis les plus récents du client ID.
...
liste des 50 factures les plus récentes du client ID.
...
| validity | Durée de validité d'un devis| int | modifiable |
...
| electronic_address | Adresse électronique PA| string(128) | modifiable |
...
|:-------|:--------|:-----|:------|
| id | Identifiant| int | lecture |
| customer_id | Client| int | modifiable |
...
| category_id | Catégorie| int | modifiable |
| followup_id | Suivi commercial| int | modifiable |
| invoiced_on | Date du devis| date | modifiable |
| term_on | Valide jusqu'au| date | modifiable |
| draft | Brouillon| boolean | lecture |
| balance_year | Exercice fiscal<br/>Indique l'année de début de l'exercice fiscal lorsque l'exercice fiscal est personnalisé.| int | lecture |
...
| created_at | Créé le| datetime(3) | lecture |
| updated_at | Modifié le| datetime(3) | lecture |
| user_id | Géré par| int | modifiable |
...
| soft_deleted | Enregistrement marqué comme à supprimer| boolean | lecture |
| hard_delete_on | Date définitive de la suppression| date | lecture |
| quote_status | Statut du devis| int | modifiable |
| quote_ref | Devis n°| int | lecture |
...
| product_id | Identifiant du produit| int | modifiable |
| position | Ligne n°| int | modifiable |
...
| style | Style de formattage<br/>Par défaut (ie valeur vide), ce champ indique une ligne de facturation standard. La valeur 'comment' permet d'indiquer qu'il s'agit d'une ligne de commentaire, la valeur 'title' permet d'indiquer une ligne de titre (mise en gras automatiquement), la value 'subtotal' permet d'indiquer une ligne de sous-total, la valeur 'new_page' permet d'indiquer un saut de page et la valeur 'subsidy' permet d'indiquer une déduction à appliquer sur le total TTC (ex: Prime CEE).| string(20) | modifiable |
| nature | Type<br/>la nature de la prestation n'est à préciser que pour les auto-entrepreneurs et les micro-entrepreneurs, afin de calculer correctement les cotisations sociales.| int | modifiable |
...
| term_on | Date d'échéance| date | modifiable |
| draft | Brouillon| boolean | lecture |
| balance_year | Exercice fiscal<br/>Indique l'année de début de l'exercice fiscal lorsque l'exercice fiscal est personnalisé.| int | modifiable |
| total | Montant total HT| decimal(15,3) | lecture |
| total_with_vat | Montant total TTC| decimal(15,3) | lecture |
| currency | Devise| string(3) | modifiable |
| rebate_percentage | Réduction globale| decimal(8,3) | modifiable |
...
| hard_delete_on | Date définitive de la suppression| date | lecture |
| delivery_on | Livré/Réalisé le| date | modifiable |
| quote_id | ID Devis associé| int | lecture |
...
| estimated_total_in_main_currency | Total TTC estimé en euro| decimal | lecture |
| refund_id | Avoir sur la facture n°ID<br/>Si la facture est un avoir généré via notre système automatisé, ce champ contient l'ID de la facture annulée| int | lecture |
...
| recurring_invoice_id | ID de la facture récurrente associée| int | lecture |
...
| measure | Unité de mesure| string(32) | modifiable |
| sap_nature | Code activité NOVA| int | modifiable |
| nature | Type de facturation| int | modifiable |
| category_id | Catégorie| int | modifiable |
| notes | Notes internes| text | modifiable |
| created_at | Créé le| datetime(3) | lecture |
| updated_at | Modifié le| datetime(3) | lecture |
...
| id | Identifiant| int | lecture |
| title | Libellé| string(255) | modifiable |
| status | Affectation| int | modifiable |
| created_at | Créé le| datetime(3) | lecture |
| updated_at | Modifié le| datetime(3) | lecture |
